...
function VendorInvoice({ vn, filterP }) {
  ...
  orders.forEach(o => {
    gp(o).filter(p => p.vendor === vn).forEach(p => {
      if (!productMap[p.name]) productMap[p.name] = { sold: 0, warehouse: 0 };
      productMap[p.name].sold += Number(p.qty) || 1;
    });
  });
  Object.values(db.inventory).forEach(branchInv => {
    const vInv = branchInv[vn] || {};
    Object.entries(vInv).forEach(([product, vals]) => {
      if (!productMap[product]) productMap[product] = { sold: 0, warehouse: 0 };
      productMap[product].warehouse += Math.max(0, (vals.received || 0) - (vals.sentOut || 0) - (vals.delivered || 0));
    });
  });
  const productRows = Object.entries(productMap).filter(([, v]) => v.sold > 0 || v.warehouse > 0);
  ...
  async function generateInvoice() {
    ...
    node.innerHTML = `
      <div style="background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.12)">
        ...
        <div style="padding:24px 36px;border-top:1px solid #eef0f7">
          ...
          <table style="width:100%;border-collapse:collapse">
            <thead>
              <tr style="background:#f6f7fb">
                <th style="text-align:left;padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#858cab;border-bottom:1px solid #eef0f7">Product</th>
                <th style="text-align:left;padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#858cab;border-bottom:1px solid #eef0f7">Qty Sold</th>
                <th style="text-align:left;padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#858cab;border-bottom:1px solid #eef0f7">In Warehouse</th>
              </tr>
            </thead>
            <tbody>
              ${productRows.map(([name, v]) => `<tr>
                <td style="padding:11px 14px;border-bottom:1px solid #f0f0f0;font-size:13px;font-weight:600;color:#0b1230">${name}</td>
                <td style="padding:11px 14px;border-bottom:1px solid #f0f0f0;font-weight:800;font-family:monospace;color:#0b1230">${v.sold}</td>
                <td style="padding:11px 14px;border-bottom:1px solid #f0f0f0;font-weight:700;font-family:monospace;color:${v.warehouse <= 0 ? '#e0425a' : '#1fa67a'}">${v.warehouse}</td>
              </tr>`).join('')}
            </tbody>
          </table>
        </div>` : ''}
    ...
    <div className="vv">
      ...
      {productRows.length > 0 && (
        <>
          ...
          <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
            <table className="tbl">
              <thead><tr><th>Product</th><th>Qty Sold</th><th>In Warehouse</th></tr></thead>
              <tbody>
                {productRows.map(([name, v]) => (
                  <tr key={name}>
                    <td style={{ fontWeight: 600 }}>{name}</td>
                    <td style={{ fontWeight: 800, fontFamily: "'JetBrains Mono',monospace" }}>{v.sold}</td>
                    <td style={{ fontFamily: "'JetBrains Mono',monospace", color: v.warehouse <= 0 ? 'var(--red)' : 'var(--green)', fontWeight: 700 }}>{v.warehouse}</td>
                  </tr>
                ))}
              </tbody>
            </table>
